...
static long int softwin_limit = 1416000000;
...
static unsigned long dvfs_get_limit_rate(struct dvfs_node *clk_dvfs_node, unsigned long rate)
{
	...
	if (clk_dvfs_node->freq_limit_en) {
		...
		if (softwin_limit && rate > softwin_limit) {
			limit_rate = softwin_limit;
			//printk("--- softwin_limit = %ld ---\n", softwin_limit);
		}
               ...
               const char *buf, size_t n)
{
	...
	{
	case 1:
		softwin_limit = 1008000000;
		break;
	case 2:
		softwin_limit = 1200000000;
		break;
	case 3:
		softwin_limit = 1416000000;
		break;
	case 4:
		softwin_limit = 1608000000;
		break;
	default :
		softwin_limit = 0;
	}
			
	printk("### value = %d softwin_limit = %ld \n", value, softwin_limit);
...
//#ifdef CONFIG_RK_CLOCK_PROC
	__ATTR(dvfs_tree,	S_IRWXU | S_IRWXG | S_IRWXO,	dvfs_tree_show,	dvfs_tree_store),
//#endif
};
