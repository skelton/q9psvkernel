...
static struct joystick_axis ps3_axis[] = {
	...
	{"left trigger",	0x30,		ABS_BRAKE},
	{"right trigger",	0x31,		ABS_GAS},
	{"RAS U2D",		ABS_RZ,		0},
};
...
int ps3_input_device(struct kp *kp)
{
	...
	kp->ps3_input_joystick->name = "Sony PLAYSTATION(R)3 Controller";
	kp->ps3_input_joystick->id.vendor = 0x054C;
	kp->ps3_input_joystick->id.product = 0x0268;
	input_set_abs_params(kp->ps3_input_joystick, 0x00, -ADC_MAX, ADC_MAX, 0, 0);
	input_set_abs_params(kp->ps3_input_joystick, 0x01, -ADC_MAX, ADC_MAX, 0, 0);
	input_set_abs_params(kp->ps3_input_joystick, 0x02, -ADC_MAX, ADC_MAX, 0, 0);
	input_set_abs_params(kp->ps3_input_joystick, 0x05, -ADC_MAX, ADC_MAX, 0, 0);
...
int xbox_input_device(struct kp *kp)
{
	...
	//360 joystick mode
	kp->xbox_input_joystick->name = "NVIDIA SHIELD Controller";
	kp->xbox_input_joystick->id.vendor = 0x0955;
	kp->xbox_input_joystick->id.product = 0x7202;
